// This tool tests for the JSON mappings in the go-weibo data types.  It will
// identify fields that are returned by the live Weibo API, but that are not
// currently mapped into a struct field of the relevant go-weibo type.  This
...
func main() {
	token := os.Getenv("WEIBO_AUTH_TOKEN")
	if token == "" {
		fmt.Println("!!! No OAuth token.  Fields tests won't run. !!!\n")
		return
	} else {
		client = weibo.NewClient(token)
	}
	...
	testStructTypes := structSlices{
		{"statuses/show.json?id=3709190205980932", &weibo.Status{}},
	}
	for _, tt := range testStructTypes {
